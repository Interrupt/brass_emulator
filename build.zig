...
pub fn build(b: *std.Build) !void {
    ...
    const app = try mach_core.App.init(b, .{
        ...
        .deps = &[_]std.build.ModuleDependency{
            .{
                .name = "ziglua",
                .module = ziglua.module("ziglua"),
            },
        },
    });
    if (b.args) |args| app.run.addArgs(args);

    app.compile.linkLibrary(ziglua.artifact("lua"));

    app.compile.addCSourceFile(.{ .file = .{ .cwd_relative = "libs/stb_image-2.28/stb_image_impl.c"}, .flags = &[_][]const u8{"-std=c99"}});
    app.compile.addIncludePath(.{ .path = "libs/stb_image-2.28"});
